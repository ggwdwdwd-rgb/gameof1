...
Список участников по времени регистрации (первый помечен отдельно):

```bash
docker compose exec server node dist/cli/remove-user.js
```

Удалить участника вместе с его устройствами, сообщениями и кодами:

```bash
docker compose exec server node dist/cli/remove-user.js --user=<ID>
```

Тем же занимается само приложение: настройки → участник → «Удалить участника»,
но только у первого зарегистрированного. Эта команда — на случай, когда в
приложение не зайти.
