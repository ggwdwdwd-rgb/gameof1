...
Регистрация по почте и паролю: экран входа сразу предлагает создать аккаунт.
Нужен `@тег` — по нему вас находят другие, общего списка участников в системе
нет (см. ARCHITECTURE.md §2.6). Найти человека можно на экране контактов: тег,
почта или телефон, только точное совпадение.

Инвайты остались как дополнительный путь и живут в командной строке сервера
(`npm run invite:create`): вошедший по коду становится контактом того, кто код
выпустил.
