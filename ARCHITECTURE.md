...
### 4.3 Роспись состава семьи при подключении
...
строит/обновляет локальную таблицу `contacts`.
...
Реализовано без отдельных типов пакетов: раздача `groupKey` едет как обычное
`msg.send` в 1:1-чат с получателем, только с `contentType: "system_group_key"`
(ciphertext — тот же групповой ключ, JSON `{chatId, keyVersion, key}`,
зашифрованный обычным парным shared key получателя). Сервер эту рассылку
вообще не отличает от обычного сообщения — не нужно ни новых типов пакетов,
ни отдельной логики маршрутизации на сервере, при этом бесплатно
получаем offline-доставку через `history.fetch`, если получатель не в сети.
Клиент, получив `msg.deliver` с этим contentType, сохраняет ключ локально
(`group_keys`) и не показывает его в списке сообщений чата.

Кто отправляет: любой участник, у которого уже есть текущий `groupKey`,
при получении `roster.snapshot`/`member.joined` отправляет его каждому
участнику из ростера, у кого этого ключа ещё может не быть (переотправка
идемпотентна — получатель просто повторно сохраняет тот же ключ). Самый
первый пользователь системы (пустой ростер при регистрации) генерирует
`keyVersion = 1` сам, остальные получают его этим механизмом при первом
подключении к сети, где онлайн хотя бы один уже состоящий в группе участник.

### 4.7 Устройства и ревокация
